...
function PendingDetails() {
  ...
  const [colDefs, setColDefs] = useState<ColDef[]>([
    ...
    { headerName:"Action" , field: "action" ,  headerClass:"font-bold border p-2 font-bold  text-md",
      ...
      <button className='text-green-500 font-bold' onClick={() => handleApproveTrainer(data.data._id,data.data.email)}>Approve</button>
      <button className='text-red-600' onClick={() => handleRejectTrainer(data.data._id,data.data.email)}>Reject</button>
    </div>},
  ])
